...
#X, y, S = sk.utils.shuffle(X, y, S, random_state=7)
...
X, dev_X, S, dev_S = train_test_split(X, S, test_size=0.30,stratify=S, random_state=1)
...
import sys
sys.stdout=open("results/vanillaNB_experimental_report.txt","w")
                ...
                for drop_prob in drop_probs:
                    for activation_func in activation_funcs:
                        for weight_decay in weight_decays:
                            ...
                            vanilla_nb = NB(input_size,latent_size,cat_size,encode_hidden,drop_prob,activation_func).to(device)
                            optimizer = optim.Adam(vanilla_nb.parameters(),lr = learning_rate,weight_decay=weight_decay)
                            ...
                            for epoch in range(num_epochs):
                                for batch in range(0,np.int64(np.floor(len(X)/minibatch))*minibatch,minibatch):
                                    ...
                                    q_z, out_beta_z0, out_beta_z1, mu_z0, mu_z1, sigma_z0, sigma_z1, log_prior_param = vanilla_nb(trainX_batch,tau)
                                    
                                    elbo = elbo_function(trainX_batch, out_beta_z0, out_beta_z1, mu_z0, mu_z1, sigma_z0, sigma_z1, vanilla_nb, numObserv, q_z, latent_prior, log_prior_param)
                                    loss = - elbo
                            ...
                            if flag == False:
                                with torch.no_grad():
                                    qz_typ, typ_out_beta_z0, typ_out_beta_z1, typ_mu_z0, typ_mu_z1, typ_sigma_z0, typ_sigma_z1, log_prior_param = vanilla_nb(devData,tau)
                                Dev_results['ELBO'][run]=elbo_function(devData, typ_out_beta_z0, typ_out_beta_z1, typ_mu_z0, typ_mu_z1, typ_sigma_z0, typ_sigma_z1, vanilla_nb, numObserv, qz_typ, latent_prior,log_prior_param)
...
np.savetxt('results/mi_per_feature_vanillaNB.txt',mi_per_feature)

#%% saving model and results
torch.save(best_net.state_dict(), "trained-models/vanilla_nb")
Dev_results.to_csv('results/Dev_results_vanillaNB.csv',index=False)