...
from objective_functions import elbo_function, held_out_logLL, stochasticEDFModel, computeEDFforBatch, df_loss
...
tau_min = 0.5
anneal_rate = 0.00003
# fixed to best typical model's hyper-parameters
encode_hidden = [64,64]
minibatch = 128
learning_rate = 0.002
drop_prob = 0.1
...
RANDOM_STATE = 160 # tuned from best typical model
...
best_vanilla_ll = -69197.594 # log-likelihood of the best vanilla model on dev set
threshold_ll = best_vanilla_ll + best_vanilla_ll*slack # to pick allowable solutions
best_fairness = 0.714 # initialize with fairness of the best vanilla model for dev set
...
coulumn_names = ['ELBO', 'MLL','CLL','avg_mi', 'epsilon', 'gamma', 'avg_gamma','dp_race', 'dp_gender', 'dp_nation', 'pRule_race', 'pRule_gender', 'pRule_nation',\
...
import sys
sys.stdout=open("results/dfNB_experimental_report.txt","w")
...
for ld in lamda_grid:
    ...
    fair_nb = NB(input_size,latent_size,cat_size,encode_hidden,drop_prob,activation_func).to(device)
    optimizer = optim.Adam(fair_nb.parameters(),lr = learning_rate,weight_decay=weight_decay)
    vb_EDFModel = stochasticEDFModel(len(intersectionalGroups),latent_size*cat_size,len(X),minibatch)
    ...
    for epoch in range(num_epochs):
        for batch in range(0,np.int64(np.floor(len(X)/minibatch))*minibatch,minibatch):
            ...
            q_z, out_beta_z0, out_beta_z1, mu_z0, mu_z1, sigma_z0, sigma_z1, log_prior_param = fair_nb(trainX_batch,tau)
            
            elbo = elbo_function(trainX_batch, out_beta_z0, out_beta_z1, mu_z0, mu_z1, sigma_z0, sigma_z1, fair_nb, numObserv, q_z, latent_prior, log_prior_param)
            loss = - elbo
            ...
            vb_EDFModel(stepSize,countClass, countTotal,len(X),minibatch)
    ...
    if flag == False:
        with torch.no_grad():
            qz_typ, typ_out_beta_z0, typ_out_beta_z1, typ_mu_z0, typ_mu_z1, typ_sigma_z0, typ_sigma_z1, log_prior_param = fair_nb(devData,tau)
        Dev_results['ELBO'][run]=elbo_function(devData, typ_out_beta_z0, typ_out_beta_z1, typ_mu_z0, typ_mu_z1, typ_sigma_z0, typ_sigma_z1, fair_nb, numObserv, qz_typ, latent_prior,log_prior_param)
        Dev_results['MLL'][run], Dev_results['CLL'][run] = held_out_logLL(devData, typ_out_beta_z0, typ_out_beta_z1, numObserv, qz_typ)
        ...
        if Dev_results['CLL'][run]>=threshold_ll and Dev_results['epsilon'][run]<best_fairness and len(np.unique(hard_cluster))>1:
...
Dev_results.to_csv('results/Dev_results_dfNB.csv',index=False)

print('Evaluation results on the test set:')
print('\n')

with torch.no_grad():
    qz_typ, typ_out_beta_z0, typ_out_beta_z1, typ_mu_z0, typ_mu_z1, typ_sigma_z0, typ_sigma_z1, log_prior_param = best_net(testData,tau)

test_elbo_typ=elbo_function(testData, typ_out_beta_z0, typ_out_beta_z1, typ_mu_z0, typ_mu_z1, typ_sigma_z0, typ_sigma_z1, best_net, numObserv, qz_typ, latent_prior, log_prior_param)
print(f"average ELBO: {test_elbo_typ: .3f}")
test_mll_typ, test_cll_typ = held_out_logLL(testData, typ_out_beta_z0, typ_out_beta_z1, numObserv, qz_typ)
print(f"average MLL: {test_mll_typ: .3f}")
print(f"average CLL: {test_cll_typ: .3f}")
...
np.savetxt('results/df_mi_per_feature_dfNB.txt',mi_per_feature)

#%% saving model and results
torch.save(best_net.state_dict(), "trained-models/df_nb")