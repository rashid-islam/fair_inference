...
from models import BernoulliVAE
                ...
                for drop_prob in drop_probs:
                    for activation_func in activation_funcs:
                        for weight_decay in weight_decays:
                            ...
                            bernoulli_vae = BernoulliVAE(input_size,latent_size,cat_size,encode_hidden,drop_prob,activation_func).to(device)
                            optimizer = optim.Adam(bernoulli_vae.parameters(),lr = learning_rate,weight_decay=weight_decay)
                            ...
                            for epoch in range(num_epochs):
                                for batch in range(0,np.int64(np.floor(len(X)/minibatch))*minibatch,minibatch):
                                    ...
                                    q_z, out_x = bernoulli_vae(trainX_batch, tau)
                            ...
                            if flag == False:
                                with torch.no_grad():
                                    qz_typ, typ_out_x = bernoulli_vae(devData, tau)
...
with torch.no_grad():
    qz_typ, typ_out_x = best_net(testData, tau)
