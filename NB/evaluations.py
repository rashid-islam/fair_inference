...
#%% mutual info for categorical variable
def mi_per_feature_discrete(X, z_hard, numObserv):
    mi_per_feature = []
    
    for obs in numObserv:
        cat_var=np.argmax(X[:,obs],axis=1)
        mi_per_feature.append(normalized_mutual_info_score(cat_var, z_hard))
    
    return np.array(mi_per_feature)
...
def groupFairnessMeasures(protectedAttributes,latent_distribution):
    ...
    # group fairness for nationality
    dp_nation = 0.0
    pRule_nation = 100.0
    
    dp_nation_avg = 0.0
    pRule_nation_avg = 0.0
    for y in range(latent_distribution.shape[1]):
        dp_temp, pRule_temp = demographicParity(S_nation, latent_distribution[:,y], dirichletAlpha, concentrationParameter)
        dp_nation_avg += dp_temp
        pRule_nation_avg += pRule_temp
        dp_nation = max(dp_nation,dp_temp)
        pRule_nation = min(pRule_nation,pRule_temp)

    return dp_race,dp_gender,dp_nation,pRule_race,pRule_gender,pRule_nation,dp_race_avg/numClasses,dp_gender_avg/numClasses,dp_nation_avg/numClasses,pRule_race_avg/numClasses,pRule_gender_avg/numClasses,pRule_nation_avg/numClasses
