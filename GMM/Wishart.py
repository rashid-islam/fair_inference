...
#%% 
# The PyTorch implementation for Wishart and Inverse-Wishart distribution is collected from the GitHub repository of the following paper:
# Pawlowski N, Coelho de Castro D, Glocker B. Deep structural causal models for tractable counterfactual inference. Advances in Neural Information Processing Systems. 2020;33:857-69.
# Link to the repository: https://github.com/biomedia-mira/deepscm/blob/master/deepscm/distributions/torch_wishart.py
...
def _posdef_logdet(A: torch.Tensor) -> torch.Tensor:
    ...
    tril = torch.linalg.cholesky(A, upper=False)
...
class Wishart(ExponentialFamily):
    ...
    @lazy_property
    def scale_tril(self):
        return torch.linalg.cholesky(self.scale, upper=False)
...
class InverseWishart(Wishart):
    ...
    def rsample(self, sample_shape=torch.Size()):
        ...
        inv_scale_tril = torch.linalg.cholesky(inv_scale, upper=False)
    ...
    def log_prob(self, value):
        if self._validate_args:
            self._validate_sample(value)
        D = self.event_shape[-1]
        value_tril = torch.linalg.cholesky(value, upper=False)
