...
from models import GaussianVFAE
...
best_vanilla_ll = -207106.656 # log-likelihood of the best vanilla model on dev set
threshold_ll = best_vanilla_ll + best_vanilla_ll*slack # to pick allowable solutions
best_fairness = 0.137 # initialize with fairness of the best vanilla model for dev set
                ...
                for drop_prob in drop_probs:
                    for activation_func in activation_funcs:
                        for weight_decay in weight_decays:
                            ...
                            gaussian_vfae = GaussianVFAE(input_size,latent_size,cat_size,group_size,encode_hidden,drop_prob,activation_func).to(device)
                            optimizer = optim.Adam(gaussian_vfae.parameters(),lr = learning_rate,weight_decay=weight_decay)
                            ...
                            for epoch in range(num_epochs):
                                for batch in range(0,np.int64(np.floor(len(X)/minibatch))*minibatch,minibatch):
                                    ...
                                    q_z, log_prob = gaussian_vfae(trainX_batch, trainS_batch, tau)
                            ...
                            if flag == False:
                                with torch.no_grad():
                                    qz_typ, typ_log_prob = gaussian_vfae(devData, PA_dev, tau)
                                ...
                                #if Dev_results['overall_dp'][run]<best_fairness and Dev_results['LL'][run]>best_ll and len(np.unique(hard_cluster))>1:
...
with torch.no_grad():
    qz_typ, typ_log_prob = best_net(testData, PA_test, tau)
