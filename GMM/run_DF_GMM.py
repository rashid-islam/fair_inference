...
RANDOM_STATE = 733  # tuned from best typical model
...
slack = 0.04 # small slack tolerance (4.0% degradation) in performance 
best_vanilla_ll = -2635074.750 # log-likelihood of the best vanilla model on dev set
threshold_ll = best_vanilla_ll + best_vanilla_ll*slack # to pick allowable solutions
best_fairness = 2.369 # initialize with fairness of the best vanilla model for dev set
...
coulumn_names = ['ELBO', 'MLL','CLL','avg_mi', 'epsilon', 'gamma', 'avg_gamma', 'dp_gender', 'dp_age', 'pRule_gender', 'pRule_age',\
...
#%% empirical prior info
from sklearn.cluster import KMeans
kmeans = KMeans(n_clusters=latent_size*cat_size, random_state=0).fit(data_eval_train)
mu_prior = kmeans.cluster_centers_
#mu_prior = ((torch.from_numpy(mu_prior)).to(device)).float()   

#%% Training fair discrete latent variable model
import sys
sys.stdout=open("results/dfGMM_experimental_report.txt","w")
...
for ld in lamda_grid:
    ...
    fair_gmm = GMM(input_size,latent_size,cat_size,encode_hidden,drop_prob,activation_func, mu_prior).to(device)
    optimizer = optim.Adam(fair_gmm.parameters(),lr = learning_rate,weight_decay=weight_decay)
    ...
    for epoch in range(num_epochs):
        for batch in range(0,np.int64(np.floor(len(X)/minibatch))*minibatch,minibatch):
            ...
            q_z, param_mu, param_covmats, log_prob = fair_gmm(trainX_batch, tau)
            
            elbo = elbo_function(q_z, param_mu, param_covmats, log_prob, latent_prior, fair_gmm)
            loss = - elbo
    ...
    if flag == False:
        with torch.no_grad():
            qz_typ, typ_param_mu, typ_param_covmats, typ_log_prob = fair_gmm(devData, tau)
        Dev_results['ELBO'][run]=elbo_function(qz_typ, typ_param_mu, typ_param_covmats, typ_log_prob, latent_prior, fair_gmm)
        Dev_results['MLL'][run], Dev_results['CLL'][run] = held_out_logLL(qz_typ, typ_log_prob)
...
Dev_results.to_csv('results/Dev_results_dfGMM.csv',index=False)

print('Evaluation results on the test set:')
print('\n')

with torch.no_grad():
    qz_typ, typ_param_mu, typ_param_covmats, typ_log_prob = best_net(testData, tau)

test_elbo_typ=elbo_function(qz_typ, typ_param_mu, typ_param_covmats, typ_log_prob, latent_prior, best_net)
print(f"average ELBO: {test_elbo_typ: .3f}")
test_mll_typ, test_cll_typ = held_out_logLL(qz_typ, typ_log_prob)
...
np.savetxt('results/df_mi_per_feature_dfGMM.txt',mi_per_feature)

#%% saving model and results
torch.save(best_net.state_dict(), "trained-models/df_gmm")