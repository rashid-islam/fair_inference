...
def groupFairnessMeasures(protectedAttributes,latent_distribution):
    ...
    S_age = np.int64(np.zeros((len(protectedAttributes))))
    ...
    pRule_age_avg = 0.0
    for y in range(latent_distribution.shape[1]):
        dp_temp, pRule_temp = demographicParity(S_age, latent_distribution[:,y], dirichletAlpha, concentrationParameter)
        dp_age_avg += dp_temp
        pRule_age_avg += pRule_temp
        dp_age = max(dp_age,dp_temp)
        pRule_age = min(pRule_age,pRule_temp)

    return dp_gender,dp_age,pRule_gender,pRule_age,dp_gender_avg/numClasses,dp_age_avg/numClasses,pRule_gender_avg/numClasses,pRule_age_avg/numClasses
