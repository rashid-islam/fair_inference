...
import sys
sys.stdout=open("results/vanillaGMM_experimental_report.txt","w")
                ...
                for drop_prob in drop_probs:
                    for activation_func in activation_funcs:
                        for weight_decay in weight_decays:
                            ...
                            vanilla_gmm = GMM(input_size,latent_size,cat_size,encode_hidden,drop_prob,activation_func,mu_prior).to(device)
                            optimizer = optim.Adam(vanilla_gmm.parameters(),lr = learning_rate,weight_decay=weight_decay)
                            ...
                            for epoch in range(num_epochs):
                                for batch in range(0,np.int64(np.floor(len(X)/minibatch))*minibatch,minibatch):
                                    ...
                                    q_z, param_mu, param_covmats, log_prob = vanilla_gmm(trainX_batch, tau)
                                    
                                    elbo = elbo_function(q_z, param_mu, param_covmats, log_prob, latent_prior, vanilla_gmm)
                                    loss = - elbo
                            ...
                            if flag == False:
                                with torch.no_grad():
                                    qz_typ, typ_param_mu, typ_param_covmats, typ_log_prob = vanilla_gmm(devData, tau)
                                Dev_results['ELBO'][run]=elbo_function(qz_typ, typ_param_mu, typ_param_covmats, typ_log_prob, latent_prior, vanilla_gmm)
                                ...
                                Dev_results['avg_mi'][run] = np.mean(mi_per_feature) # median instead of mean as there are lots of attributes in this data
...
np.savetxt('results/mi_per_feature_vanillaGMM.txt',mi_per_feature)

#%% saving model and results
torch.save(best_net.state_dict(), "trained-models/vanilla_gmm")
Dev_results.to_csv('results/Dev_results_vanillaGMM.csv',index=False)